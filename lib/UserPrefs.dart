...
class UserPrefs {
  ...
  static const _keyDaysObj = 'daysObj';
  static const _key = 'key';
  static const _keyOk = 'ok';
  ...
  // days to quit smoking
  static Future setDaysObj(int daysObj) async =>
      await _preferences?.setInt(_keyDaysObj, daysObj);
  static int? getDaysObj() => _preferences?.getInt(_keyDaysObj);
  // key to use if the user finishes his program
  static Future setKey(int key) async => await _preferences?.setInt(_key, key);
  static int? getKey() => _preferences?.getInt(_key);
  // key used to set the days without smoking to 0
  static Future setOk(int ok) async => await _preferences?.setInt(_keyOk, ok);
  static int? getOk() => _preferences?.getInt(_keyOk);
}
